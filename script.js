...
function showModal(modalId) {
  ...
  if (modalId === 'advertisementModal' || modalId === 'advertisementSidebar') {
    // Встановлюємо таймер для показу кнопки закриття
    setTimeout(() => {
      document.getElementById('closeAdSidebarBtn').disabled = false;
      document.getElementById('closeAdSidebarBtn').style.display = 'block'; // Показуємо кнопку
    }, 5000); // Час очікування перед показом хрестика
  }
}

// Нова функція для закриття бічної панелі реклами
function closeSidebarModal(modalId) {
  const modal = document.getElementById(modalId);
  modal.style.display = 'none';
}

// Викликати showModal для показу реклами при завантаженні сторінки
window.onload = function() {
  showModal('advertisementSidebar');
};


// Функція для показу рекламного банера
function showAdBanner() {
  const adBanner = document.getElementById('adBanner');
  adBanner.style.display = 'block'; // Показати банер

  // Зробити кнопку закриття активною через 5 секунд
  setTimeout(() => {
    const closeBtn = document.getElementById('closeAdBannerBtn');
    closeBtn.disabled = false;
  }, 5000);
}

// Функція для закриття рекламного банера
function closeAdBanner() {
  const adBanner = document.getElementById('adBanner');
  adBanner.style.display = 'none'; // Приховати банер
}

// Викликати функцію showAdBanner при завантаженні сторінки
window.onload = function() {
  showAdBanner();
};